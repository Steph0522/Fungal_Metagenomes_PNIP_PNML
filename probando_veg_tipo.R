...
#datos totales promedios y medianas por tipo
...
means_genus<-function(y){veg_group %>% dplyr::select(SampleID, Tipo,y) %>%  pivot_wider(
...
library(phyloseq)
data_spet_physeq <- phyloseq(otu_table(data_spet, taxa_are_rows = F))
data_spet_dist<-as.matrix(phyloseq::distance(data_spet_physeq, method="bray"))

mantel(bc.dist[[1]], data_spet_dist)
cor.test(bc.dist[[1]], data_spet_dist)
...
dim(mm3)
head(mm3)
env.16S3=mm3[1:26] %>% column_to_rownames(var = "SampleID")
dim(env.16S3)
colnames(env.16S3)
spp.16S3=mm3[c(1,27:98)]%>% column_to_rownames(var = "SampleID")
colnames(spp.16S3)
...
dim(mm1)
head(mm1)
env.16S1=mm1[1:26] %>% column_to_rownames(var = "SampleID")
colnames(env.16S1)
spp.16S1=mm1[c(1,27:236)]%>% column_to_rownames(var = "SampleID")

#paired
mm2=env.16S%>%  inner_join(spp.16S2 %>%rownames_to_column(var = "SampleID") )
dim(mm2)
head(mm2)
env.16S2=mm2[1:26] %>% column_to_rownames(var = "SampleID")
colnames(env.16S2)
spp.16S2=mm2[c(1,27:260)]%>% column_to_rownames(var = "SampleID")

#qiime2
mm4=env.16S%>%  inner_join(spp.16S4 %>%rownames_to_column(var = "SampleID") )
dim(mm4)
head(mm4)
env.16S4=mm2[1:26] %>% column_to_rownames(var = "SampleID")
dim(env.16S4)
spp.16S2=mm4[c(1,27:3164)]%>% column_to_rownames(var = "SampleID")
...
vars_chose<-c("mean_Altura_Coníferas",   "mean_areabasal_Coníferas" ,
              "total_mean_coberturacopa" )

c<- read.csv("/home/yendi/Desktop/envs.csv", row.names = 1)

df<- env.16S %>% dplyr::select(-mean_Altura_Latifoliadas)
df<- env.16S %>% dplyr::select(vars_chose, SampleID)

dfss<- merge(env.16S, dfsA, by = "SampleID") %>% dplyr::select(-mean_Altura_Latifoliadas)
...
df1<-env.16S1 %>% dplyr::select(mean_CopaNS_Abies,mean_Altura_Pinus,
                                mean_areabasal_Salix,mean_DAP_Pinus)  %>% 
  dist(., method = "euclidean")
df1<-env.16S1 %>% dplyr::select(rownames(c1))  %>% 
  dist(., method = "euclidean")
df2<-env.16S2 %>% dplyr::select(prop_Arbutus,mean_Altura_Pinus,
                                mean_Altura_Abies,mean_CopaNS_Quercus  )  %>% 
  dist(., method = "euclidean")
...
df3<- env.16S3 %>% dplyr::select(mean_distanciacopas_Pinus, mean_CopaNS_Salix) %>% 
  dist(., method = "euclidean")
df4<- env.16S3 %>% dplyr::select("mean_areabasal_Abies",
"mean_areabasal_Salix", "mean_Altura_Prunus"  ) %>% 
  dist(., method = "euclidean")
...
mantel(bc.dist2[[1]], dfs_distA)
mantel(bc.dist2[[2]], dfs_distA)
mantel(bc.dist2[[3]], dfs_distA)
mantel(bc.dist2[[4]], dfs_distA)

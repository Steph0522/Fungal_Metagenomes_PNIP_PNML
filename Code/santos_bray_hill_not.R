...
dfs1=data.frame(df[1],scale(df[,2:17], center = T, scale = T)) %>% dplyr::select(
  SampleID,K, LIMO,Ca, Mg) %>% 
  column_to_rownames(var = "SampleID")
dfs2=data.frame(df[1],scale(df[,2:17], center = T, scale = T)) %>% dplyr::select(
  SampleID,P,K,Ca,Mg,moisture,WHC, LIMO) %>% 
  column_to_rownames(var = "SampleID")
dfs3=data.frame(df[1],scale(df[,2:17], center = T, scale = T)) %>% dplyr::select(
  SampleID,P,K,Ca,Mg,moisture,WHC, LIMO) %>% 
  column_to_rownames(var = "SampleID")
dfs4=data.frame(df[1],scale(df[,2:17], center = T, scale = T)) %>% dplyr::select(
  SampleID,P,K,Ca,Mg,moisture,WHC, LIMO) %>% 
  column_to_rownames(var = "SampleID")

dfs_dist<- dist(dfs, method = "euclidean")
dfs_dist1<- dist(dfs1, method = "euclidean")
...
mantel(bc.dist[[1]], dfs_dist)
mantel(bc.dist2[[1]], dfs_dist1)

    ...
    data_paired_order)%>% full_join(
      data_fungi_order) %>% replace(is.na(.), 0)  %>% mutate(gen=case_when(
        ...
        var = "gen") %>% mutate(proms=rowMeans(.)) %>% dplyr::arrange(-proms) %>% slice(
          1:20)%>% dplyr::select(-proms) %>% mutate_all(., funs(R = case_when(
          . <= 0.001 ~ 0,
          . >  0.001 & .  <= 0.005 ~ 1,
          . >  0.005 & .  <= 0.01 ~ 2,
          . >  0.01 & .  <= 0.10 ~ 3,
...
heats<-ComplexHeatmap::Heatmap(
  ...
  width = ncol(merge_data)*unit(2.5, "mm"), 
    height = nrow(merge_data)*unit(4, "mm"),
...
draw(heats, heatmap_legend_side = "bottom",
     annotation_legend_side = "top", 
     merge_legend=F,
     annotation_legend_list = list(lgd1))


heatm<-grid.grabExpr(draw(heats, heatmap_legend_side = "bottom",
                          annotation_legend_side = "top", 
                          merge_legend=F,
                          annotation_legend_list = list(lgd1)))
                     
heatm
ggsave('heat_final_sem.png', width = 17, height = 10, dpi = 300, plot =heatm)

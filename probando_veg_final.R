...
colnames(env.16S4)
spp.16S4=mm4[c(1,16:87)]%>% column_to_rownames(var = "SampleID")
...
set.seed(122)
cap.env1=capscale(spp.16S_hell1~.,env.16S_st1[varschose],distance = "bray")
mod0.env=capscale(spp.16S_hell1~1,env.16S_st1,distance = "bray")
step.env=ordistep(mod0.env,scope = formula(cap.env1))
cap1<-step.env$anova %>% mutate(method="QIIME2") # These are the selected structuring variables that are included in the subsequent analyses 

cap.env2=capscale(spp.16S_hell2~.,env.16S_st2[varschose],distance = "bray")
...
cap.env3=capscale(spp.16S_hell3~.,env.16S_st3[varschose],distance = "bray")
...
cap.env4=capscale(spp.16S_hell4~.,env.16S_st4[varschose],distance = "bray")
mod0.env=capscale(spp.16S_hell4~1,env.16S_st4,distance = "bray")
step.env=ordistep(mod0.env,scope = formula(cap.env4))
cap4<-step.env$anova %>% mutate(method="Kraken2")#ninguno 
...
dfs_dist<- dist(dfs, method = "bray")

dfsA=dfss %>% 
 column_to_rownames(var = "SampleID")
dfs_distA<- dist(dfsA, method = "bray")
...
corrplot(corss,  method = 'circle', type = 'lower', insig='blank',
 ...
 # corss[upper.tri(corss)] <- NA
  ...
  #write.csv(as.data.frame(cor2), "Data/cor_veg_df6.csv")

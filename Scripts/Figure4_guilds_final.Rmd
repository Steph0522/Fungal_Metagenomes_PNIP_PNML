...
meta<-metadata
guild_colors<- read_csv("../Data/guilds_color_final.txt")
trophic_colors<- read_csv("../Data/trophic_color_final.txt")
...
barplot_guild<-bar_data%>% filter(Guilds %in% chose_guild ) %>%
  ...
  #scale_fill_manual(name = "Guilds",values =col_guild )+
  scale_fill_carto_d(name="Guilds", palette = "Safe")+
  ...
  mutate(
    ...
    Site= case_when(
      ...
      Sitio==2~ "S2"))%>% ggbarplot(
        ...
        position = position_fill()) +facet_nested(
          .~method, scales = "free_x")+
  #scale_fill_manual(name = "Trophic",values =col_trophic )+
scale_fill_carto_d(name="Guilds", palette = "Safe")+
theme_linedraw()+ylab("Relative abundance")+
    xlab("")+theme(legend.text = element_text(face = "plain"))+
  #  guides(fill = guide_legend(nrow = 30))+
...
all
ggsave("Figure 4_2.pdf",width = 12, height = 7, dpi = 600, plot = all, device = "pdf")

...
a1<-z+theme_linedraw()+
 ...
 # scale_color_viridis_d(option ="turbo" )+#color of points +
  ...
  label.padding = unit(0.1, "lines"), label.size = 0.2, 
     size=3.5,   check_overlap = TRUE,  color="white",fill="black", fontface="bold",
 )+
  ggrepel::geom_label_repel(data = fdat %>% filter(
    score=="biplot") %>%  
      mutate(CAP1=CAP1*0.5, CAP2=CAP2*0.5) ,
    aes(x=CAP1, y=CAP2, label= label),fontface="bold",
   segment.color = NA, size=4.5,  box.padding = 0.1, label.padding = 0.1)+
  theme_linedraw() +
...
a1<-z+theme_linedraw()+
 ...
 # scale_color_viridis_d(option ="turbo" )+#color of points +
  ...
  ggrepel::geom_label_repel(data = fdat %>% filter(
    ...
    aes(x=CCA1, y=CCA2, label= label),fontface="bold",
   segment.color = NA, size=4.5,  box.padding = 0.1, label.padding = 0.1)+
  theme_linedraw() +
  theme(axis.text = element_text(colour = "black", size = 5),
        axis.title = element_text(colour = "black", size = 10),
...
as2<- plot_grid(as,leg,ncol = 2, rel_widths = c(.93,.07), align = "hv")
as2
ggsave("Figure7.cca_ggplot_env.pdf",width = 12, height =8.5, dpi = 300, plot = as2, device = "pdf")
 ...
 # scale_color_viridis_d(option ="turbo" )+#color of points +
  ...
  ggrepel::geom_label_repel(data = fdat %>% filter(
    score=="biplot") %>%  
      mutate(CAP1=CAP1*0.5, CAP2=CAP2*0.5) ,
    aes(x=CAP1, y=CAP2, label= label),fontface="bold",
   segment.color = NA, size=4,  box.padding = 0.1, label.padding = 0.1)+
  theme_linedraw() +
  theme(axis.text = element_text(colour = "black", size = 5),
        axis.title = element_text(colour = "black", size = 10),
        legend.text = element_text(size = 10),

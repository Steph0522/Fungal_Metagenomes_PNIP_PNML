...
guild_qiime2<- read.delim(
  ...
  !Guild=="-",! `Confidence Ranking`=="Possible" , !Guild=="NULL") %>% mutate(Method="QIIME2")
...
g1<- guild_qiime2 %>% group_by(Guild) %>% summarise_if(
  is.numeric, sum) %>% column_to_rownames(
    var = "Guild") %>% relabunda(.) %>% rownames_to_column(var = "Guild") %>% mutate(method="QIIME2") %>%  mutate(total = rowMeans(across(where(is.numeric)))) %>% top_n(n = 10, wt = total) %>% dplyr::select(-total)
...
library(cowplot)
all<- plot_grid(
  barplot_guild+theme(legend.text = element_text(size = 7),   legend.title = element_text(face = "bold")),                  barplot_trophic+theme(legend.text = element_text(size = 8.5),  legend.title = element_text(face = "bold")), nrow = 2, rel_heights = c(1.2,1))
all
ggsave("guilds_final4.png",width = 11, height = 7, dpi = 300, plot = all, device = "png")
